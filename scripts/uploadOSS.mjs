...
async function uploadFile(objectName, withHash = false) {
  const file = resolve(buildPath, objectName);
...
async function main() {
  // 首先上传不带 hash 的文件
  for await (const entry of readdirp(buildPath, { depth: 0, type: "files" })) {
    queue.add(() => uploadFile(entry.path));
    // uploadFile(entry.path)
  }
  // 上传携带 hash 的文件
  for await (const entry of readdirp(`${buildPath}/assets`, { type: "files" })) {
