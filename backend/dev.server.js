...
function registerHotKeys(app) {
  app.get("/api/getHotKeys", (req, res) => {
    res.json({
      code: ERR_OK,
      result: {
        hotKeys: ["缺氧", "爱都爱了", "爱情废柴", "遗失的心跳", "小城夏天", "BLACKPINK", "苍兰诀"].map((key, index) => {
          return {
            key: key,
            id: index,
          };
...
function registerSearchV2(app) {
  app.get("/api/search-v2", (req, res) => {
    const { query, page, showSinger } = req.query;
    const data = {
      _: +new Date(),
      cv: "4747474",
      ...
      get(url, data)
        .then(({ data: { code, data } }) => {
          if (code === ERR_OK) {
            ...
            const singerList = singer.itemlist || {};
            const songList = song.itemlist || {};
            const [singerInfo] = singerList;
            res.json({
              code: ERR_OK,
              result: {
                songs: songList,
...
app.listen(PORT, HOST, () => console.log(`接口服务器开启成功 - ${HOST}:${PORT}`));
